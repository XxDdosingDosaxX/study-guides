...
/* ============================================================
   SCORE CALCULATORS (BISAP, Ranson)
   ============================================================ */
...
function interpBisap(t,box){
  var col,rec;
  if(t<=2){ col='#34d399'; rec='Low risk (BISAP 0–2) — mortality <2%. Manage on the floor with goal-directed fluids; reassess severity at 24 & 48 h.'; }
  else { col='#f43f5e'; rec='High risk (BISAP 3–5) — mortality >15%. Consider ICU/step-down, close monitoring, and early escalation.'; }
  box.querySelector('.score-total').style.color=col;
  box.querySelector('.score-interp').innerHTML='<b style="color:'+col+'">'+rec+'</b><br>Wu et al., Gut 2008 (assess within first 24 h).';
}
function interpRanson(t,box){
  var col,rec;
  if(t<=2){ col='#34d399'; rec='Score 0–2 — mortality ~0–3%. Predicted mild course.'; }
  else if(t<=4){ col='#f59e0b'; rec='Score 3–4 — mortality ~15%. Predicted severe; monitor closely, consider higher level of care.'; }
  else if(t<=6){ col='#fb7185'; rec='Score 5–6 — mortality ~40%. Severe disease; ICU-level care.'; }
  else { col='#f43f5e'; rec='Score ≥7 — mortality ~50% to near-100%. Critical; aggressive ICU management.'; }
  box.querySelector('.score-total').style.color=col;
  box.querySelector('.score-interp').innerHTML='<b style="color:'+col+'">'+rec+'</b><br>≥3 suggests severe AP (completes at 48 h; non-gallstone thresholds shown).';
}

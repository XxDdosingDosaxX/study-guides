...
/* ============================================================
   IDSA SEVERITY CLASSIFIER  (WBC + Cr + fulminant feature)
   ============================================================ */
function calcSeverity(){
  var wbc=parseFloat(document.getElementById('sevWbc').value);
  var cr=parseFloat(document.getElementById('sevCr').value);
  var ful=document.getElementById('sevFul').checked;
  var out=document.getElementById('sevOut'), rx=document.getElementById('sevRx');
  if(isNaN(wbc)||isNaN(cr)){ out.textContent='—'; out.style.color='var(--ctx)'; rx.textContent='Enter WBC and creatinine.'; return; }
  var severeLabs = (wbc>=15000)||(cr>1.5);
  if(ful){
    out.textContent='FULMINANT'; out.style.color='#f43f5e';
    rx.innerHTML='<strong style="color:#f43f5e">Vancomycin 500 mg PO/NG QID + metronidazole 500 mg IV q8h</strong>; add vancomycin retention enema 500 mg in ~100 mL NS PR q6h if ileus. <strong>ICU + early surgery consult.</strong>';
  } else if(severeLabs){
    out.textContent='SEVERE'; out.style.color='#f59e0b';
    rx.innerHTML='<strong style="color:#f59e0b">Vancomycin 125 mg PO QID × 10 d</strong> (or fidaxomicin 200 mg PO BID × 10 d). Admit; not metronidazole. Trend WBC/Cr/lactate.';
  } else {
    out.textContent='NON-SEVERE'; out.style.color='#34d399';
    rx.innerHTML='<strong style="color:#34d399">Fidaxomicin 200 mg PO BID × 10 d</strong> (preferred) or vancomycin 125 mg PO QID × 10 d. Metronidazole only if neither available.';
  }
}
function initSevCalc(){
  ['sevWbc','sevCr'].forEach(function(id){var e=document.getElementById(id); if(e) e.addEventListener('input',calcSeverity);});
  var f=document.getElementById('sevFul'); if(f) f.addEventListener('change',calcSeverity);
  if(document.getElementById('sevOut')) calcSeverity();
}

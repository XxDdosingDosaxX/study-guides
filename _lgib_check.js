...
function interpOakland(t,box){
  var groups=['age','sex','prev','dre','hr','sbp','hb'];
  var complete=groups.every(function(g){return box.querySelector('.calc-opt[data-group="'+g+'"].sel');});
  var col,rec;
  if(!complete){ col='var(--ctx)'; rec='Select one option per row to compute the Oakland score.'; }
  else if(t<=8){ col='#34d399'; rec='<b>Score ≤8 — low risk.</b> ~95% probability of safe discharge; consider outpatient management with early colonoscopy (BSG 2019).'; }
  else { col='#f43f5e'; rec='<b>Score &gt;8 — not low risk.</b> Admit for inpatient management, resuscitation, and risk-appropriate monitoring.'; }
  box.querySelector('.score-total').style.color=col;
  box.querySelector('.score-interp').innerHTML='<span style="color:'+col+'">'+rec+'</span>';
}

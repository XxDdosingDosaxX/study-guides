...
/* ============================================================
   COSYNTROPIN STIM INTERPRETER
   ============================================================ */
(function(){
  var inp=document.getElementById('stimVal');
  if(!inp) return;
  function render(){
    var v=parseFloat(inp.value);
    var out=document.getElementById('stimOut'), msg=document.getElementById('stimMsg');
    if(isNaN(v)){ out.textContent='—'; out.style.color='var(--ctx)'; msg.textContent='Enter a value.'; return; }
    out.textContent=v.toFixed(1)+' µg/dL';
    if(v>=18){ out.style.color='#34d399'; msg.innerHTML='<strong>Normal response — AI effectively excluded.</strong> Peak ≥18–20 µg/dL means the adrenal responded. Consider very-early secondary AI if clinical suspicion remains high.'; }
    else if(v>=14){ out.style.color='#f59e0b'; msg.innerHTML='<strong>Indeterminate / assay-dependent.</strong> On newer monoclonal (LC-MS/MS) assays ~14–15 µg/dL may be the cutoff; on polyclonal assays this is subnormal. Correlate clinically and confirm your lab’s threshold.'; }
    else { out.style.color='#f43f5e'; msg.innerHTML='<strong>Subnormal — consistent with adrenal insufficiency.</strong> Peak &lt;18 µg/dL. Localize with plasma ACTH (high = primary, low/normal = secondary) and start replacement.'; }
  }

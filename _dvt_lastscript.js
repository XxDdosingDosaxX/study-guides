...
/* ============================================================
   SCORE CALCULATORS (Wells DVT + Wells PE)
   ============================================================ */
...
function computeScore(box){
  var t=0; box.querySelectorAll('.calc-opt.sel').forEach(function(o){t+=parseFloat(o.dataset.pts||'0');});
  var tot=box.querySelector('.score-total'); if(tot) tot.textContent=(Math.round(t*10)/10);
  var fn=window[box.dataset.interp]; if(fn) fn(t,box);
}
function interpWellsDvt(t,box){
  var col,rec;
  if(t<2){col='#34d399';rec='DVT UNLIKELY (≤1) — order a high-sensitivity D-dimer (age-adjusted if &gt;50). Negative → DVT excluded; positive → compression ultrasound.';}
  else {col='#f43f5e';rec='DVT LIKELY (≥2) — go straight to compression ultrasound. Do NOT rely on D-dimer to exclude.';}
  box.querySelector('.score-total').style.color=col;
  box.querySelector('.score-interp').innerHTML='<b style="color:'+col+'">'+rec+'</b><br>Two-tier Wells (Wells 2003, NEJM).';
}
function interpWellsPe(t,box){
  var col,rec;
  if(t<=1){col='#34d399';rec='LOW pre-test probability (&lt;2). Two-tier: ≤4 = "PE unlikely" → consider D-dimer / PERC.';}
  else if(t<=6){col='#f59e0b';rec='MODERATE probability (2–6). If two-tier ≤4 → D-dimer; if &gt;4 → CT pulmonary angiography.';}
  else {col='#f43f5e';rec='HIGH probability (&gt;6) / "PE likely" (&gt;4) → CT pulmonary angiography (skip D-dimer).';}
  box.querySelector('.score-total').style.color=col;
  box.querySelector('.score-interp').innerHTML='<b style="color:'+col+'">'+rec+'</b><br>Original 3-tier + two-tier (Wells 2001, Ann Intern Med).';
}
